...
def dig(moonbase: Moonbase): Moonbase = { 
  val findLowest = listLowestBases(moonbase.bases) = findLowest match {
    case Nil => Nil
    case h :: t => Moonbase(moonbase.bases, moonbase.resources + (h._1*25))
  }
}



def compressLevelBases(bases: List[Bases]): List(Int, Int) = bases match{
